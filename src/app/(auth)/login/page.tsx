...
export default function LoginPage() {
  ...
  const loginMutation = useMutation({
    mutationFn: async (values: LoginInput) => {
      ...
      if (res.error) throw new Error(res.error.message ?? 'Pogrešna email adresa ili lozinka');
      return res.data;
    },
    onSuccess: () => {
    ...
    <main className="bg-mesh-soft relative flex min-h-dvh flex-col overflow-hidden">
      ...
      <div className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center px-5 pb-16">
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          <h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">Dobrodošao nazad</h1>
          <p className="text-muted-foreground mt-2 max-w-xs text-sm">
            Prijavi se da nastaviš sa uplatnicama.
          </p>
        </div>
        ...
        <div className="border-border bg-card shadow-card-lg relative overflow-hidden rounded-3xl border p-6 sm:p-8">
          ...
          <p className="text-muted-foreground mt-6 text-center text-sm">
            Nemaš nalog?{' '}
            <Link href="/register" className="text-foreground hover:text-brand font-medium">
              Registruj se
            </Link>
          </p>
        </div>
